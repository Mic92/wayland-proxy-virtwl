...
let make_relative_pointer ~host_relative_pointer c =
  let h =
    host_relative_pointer @@ object
      ...
      method on_relative_motion _ ~utime_hi ~utime_lo ~dx ~dy ~dx_unaccel ~dy_unaccel =
        C.Zwp_relative_pointer_v1.relative_motion c ~utime_hi ~utime_lo ~dx ~dy ~dx_unaccel ~dy_unaccel
    end
  in
  Proxy.Handler.attach c @@ object
...
let make_relative_pointer_manager bind proxy =
  ...
  Proxy.Handler.attach proxy @@ object
    ...
    method on_get_relative_pointer _ relative_pointer ~pointer =
      make_relative_pointer ~host_relative_pointer:(H.Zwp_relative_pointer_manager_v1.get_relative_pointer h ~pointer:(to_host pointer)) relative_pointer
  end
